      ...
      <section className="leadership-section" id="leadership">
        <div className="shell">
          ...
          <div className="leadership-panels">
            <div className="committee-panel"><div className="panel-title"><h3>Executive committee</h3><ShipWheel /></div><div className="people-grid">{committee.map((person) => <article className="person" key={person.name}><div className={`person__avatar ${person.photo?"has-photo":""}`}>{person.photo?<img src={person.photo} alt=""/>:<span>{person.initials}</span>}</div><strong>{person.name}</strong><span>{person.role}</span></article>)}</div></div>
            <div className="advisory-panel"><div className="panel-title"><h3>Advisory panel</h3><Sparkles /></div><div className="people-grid">{advisors.map((person) => <article className="person person--light" key={person.name}><div className={`person__avatar person__avatar--line ${person.photo?"has-photo":""}`}>{person.photo?<img src={person.photo} alt=""/>:<span>{person.initials}</span>}</div><strong>{person.name}</strong><span>{person.role}</span></article>)}</div></div>
          </div>
        </div>
      </section>
...
export function DirectoryPage({ boats }: { boats: Boat[] }) {
  ...
  const filtered = useMemo(() => {
    const normalized = query.toLowerCase().trim();
    const result = boats.filter((boat) => (`${boat.name} ${boat.owner} ${boat.membership}`.toLowerCase().includes(normalized)) && (type === "All types" || boat.type === type) && (district === "All districts" || boat.district === district));
