      ...
      <section className="leadership-section" id="leadership">
        <div className="shell">
          ...
          <div className="leadership-panels">
            <div className="committee-panel"><div className="panel-title"><h3>Executive committee</h3><ShipWheel /></div><div className="people-grid">{committee.map((person) => <article className="person" key={person.name}><div className={`person__avatar ${person.photo?"has-photo":""}`}>{person.photo?<img src={person.photo} alt=""/>:<span>{person.initials}</span>}</div><strong>{language === "bn" && person.nameBn ? person.nameBn : person.name}</strong><span>{language === "bn" && person.roleBn ? person.roleBn : person.role}</span></article>)}</div></div>
            <div className="advisory-panel"><div className="panel-title"><h3>Advisory panel</h3><Sparkles /></div><div className="people-grid">{advisors.map((person) => <article className="person person--light" key={person.name}><div className={`person__avatar person__avatar--line ${person.photo?"has-photo":""}`}>{person.photo?<img src={person.photo} alt=""/>:<span>{person.initials}</span>}</div><strong>{language === "bn" && person.nameBn ? person.nameBn : person.name}</strong><span>{language === "bn" && person.roleBn ? person.roleBn : person.role}</span></article>)}</div></div>
          </div>
        </div>
      </section>
