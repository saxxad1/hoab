  ...
  return (
    <main>
      ...
      <section className="application-section"><div className="shell application-layout">
        <aside className="application-intro"><div className="b2b-icon b2b-icon--light"><Building2 /></div><h2>A clearer route to trusted partnerships.</h2><p>Approved agencies are listed in HOAB&apos;s public directory and connected to a verified houseboat network.</p><ul><li><Check /> Official recognition</li><li><Check /> Verified operator network</li><li><Check /> Structured business rules</li><li><Check /> Tourism-event opportunities</li></ul><div className="help-box"><strong>Need help?</strong><span style={{ wordBreak: "break-all" }}>houseboatownersassociation70@gmail.com</span><span>+880 1700 123 456</span></div></aside>
        <div className="application-card">
          ...
          <form onSubmit={next}>
            ...
            {step === 2 && <fieldset><legend>Supporting documents</legend><p className="form-lead">Upload clear PDF, JPG or PNG files. Private documents are visible only to authorised reviewers.</p><div className="upload-grid"><label className="upload-card"><Upload /><strong>{files.tradeLicense?.name || "Trade license"}</strong><span>PDF, JPG or PNG · max 8 MB</span><input required type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFiles((current) => ({ ...current, tradeLicense: e.target.files?.[0] ?? null }))} /></label><label className="upload-card"><Upload /><strong>{files.associationCertificate?.name || "Association certificate"}</strong><span>PDF, JPG or PNG · max 8 MB</span><input required type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFiles((current) => ({ ...current, associationCertificate: e.target.files?.[0] ?? null }))} /></label><label className="upload-card"><Upload /><strong>{files.nidDocument?.name || "Responsible person NID"}</strong><span>Front and back in one file</span><input required type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFiles((current) => ({ ...current, nidDocument: e.target.files?.[0] ?? null }))} /></label><label className="upload-card upload-card--optional"><FileText /><strong>{files.additionalDocument?.name || "Additional document"}</strong><span>Optional supporting file</span><input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(e) => setFiles((current) => ({ ...current, additionalDocument: e.target.files?.[0] ?? null }))} /></label></div></fieldset>}
