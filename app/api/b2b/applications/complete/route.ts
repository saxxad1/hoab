...
export async function POST(request: Request) {
  try {
    ...
    if (documents.length) {
      ...
      if (documents.some((document) => !storedNames.has(document.storageKey))) return Response.json({ error: "One or more selected documents did not finish uploading. Please try again." }, { status: 409 });
    }
