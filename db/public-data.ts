...
export async function getPublicData(): Promise<PublicData> {
  if (!process.env.DATABASE_URL) return getDemoPublicData();
  try {
    const db = getDb();
    ...
    return getDemoPublicData();
  }
}

export async function getPublicBoat(slug: string): Promise<Boat | null> {
  if (!process.env.DATABASE_URL) return getDemoPublicData().boats.find((boat) => boat.slug === slug) ?? null;
  try {
    ...
    console.error("Database lookup failed for boat slug, falling back:", err);
    return getDemoPublicData().boats.find((boat) => boat.slug === slug) ?? null;
  }
}
